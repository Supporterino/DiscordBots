...
export class MovementBot extends BasicBot {
  ...
  moveTo(msg: Message) {
    if (!this.getSingleMention(msg)) msg.reply('You need to mention a target!');
      ...
      const channelTarget = msg.content
        .split(this.getSingleMention(msg).user.toString().substring(3))
        [msg.content.split(this.getSingleMention(msg).user.toString().substring(3)).length - 1].trim();
